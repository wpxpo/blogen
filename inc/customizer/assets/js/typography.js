jQuery(document).ready(function($){ 'use strict';
    ...
    let font    = 'default',
        type    = 'default',
        weight  = 'default';
    let value   = $('.mainsite-customizer-typography-input').val()
    if ( value ) {
        value   = value.split("#");
        font    = value[0] || 'default';
        type    = value[1] || 'default';
        weight  = value[2] || 'default';
        if( font ){
            dataSelect =  data.filter( val => val.n == font );
            $('.mainsite-field-typography-weight-field').html( (dataSelect[0].v).map( val => '<option '+(val==weight?' selected':'')+'>' + val + '</option>' ) );
        }
    }
    
    $('.mainsite-field-typography-field').append( data.map( val => '<option'+(val.n==font?' selected':'')+'>' + val.n + '</option>' ) );
    
    $('.mainsite-field-typography-field').on('change', function() {
        font = this.value
        dataSelect =  data.filter( val => val.n == font );
        type = dataSelect[0].f
        $(this).parents('.mainsite-customizer-typography').find('.mainsite-field-typography-weight-field').html( (dataSelect[0].v).map( val => '<option>' + val + '</option>' ) );
    });
    $('.mainsite-field-typography-field, .mainsite-field-typography-weight-field').on('change', function() {
        const selector = $(this).parents('.mainsite-customizer-typography').find('.mainsite-customizer-typography-input')
        selector.val( font + '#' + type + '#' + ($(this).parents('.mainsite-customizer-typography').find('.mainsite-field-typography-weight-field').val()||'default') )
